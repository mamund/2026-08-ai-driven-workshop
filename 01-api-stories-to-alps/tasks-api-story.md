 ...
 * **Home** : The home or landing page of the API. Users typically start here.
   * Actions: **GetTaskCollection**, **GetFilteredTaskCollection**, **ShowHomePage**
 * **TaskCollection** : The list of tasks in the system are displayed here.
   * Actions: **ShowHomePage**, **GetTaskCollection**, **GetFilteredTaskCollection**, **CreateNewTask**,**GetTaskItem**
 * **TaskItem** : This resource shows a single task (selected from the TaskCollection)
   * Actions: **EditExistingTask**, **UpdateStatusOfTask**, **SetDueDateOfTask**, **AssignUserToTask**, **GetTaskCollection**, **GetFilteredTaskCollection**, **ShowHomePage**
 ...
 * **GetFilteredTaskCollection** : Use this action to filter the list of tasks by `title`, `dueDate`, `status`, `priority`  and/or `assignedUser`
   * Inputs: title, dueDate,status, priority, assignedUser
   * Returns: **TaskCollection**
   * Type: Safe

## Rules
 * Each Task record MUST have a unique `id`.
 * When executing **CreateNewTask**, if the client does not supply an `id` value, the service MUST generate one.
 * **CreateNewTask** MUST reject a client-supplied `id` value that is already in use.
